...

{
    ...
    // --- 2. Desenhar o Contorno (Borda) ---
    juce::Rectangle<float> lowMidContourArea(
        // X (Início): Ponto médio entre Low e LowMid
        division1X,
        // Y (Topo): O topo do mainPanelArea
        topY,
        // Largura: A distância entre a Divisão 1 e a Divisão 2
        division2X - division1X,
        // Altura: A distância entre a Base e o Topo
        angleBaseY - topY
    );

    g.drawRoundedRectangle(lowMidContourArea, bandRadius, contourThickness);
    juce::Rectangle<float> highMidContourArea(
        // X (Início): Ponto médio entre LowMid e HighMid
        division2X,
        // Y (Topo): O topo do mainPanelArea
        topY,
        // Largura: A distância entre a Divisão 2 e a Divisão 3
        division3X - division2X,
        // Altura: A distância entre a Base e o Topo
        angleBaseY - topY
    );
    g.drawRoundedRectangle(highMidContourArea, bandRadius, contourThickness);

    // --- II. Desenhar Ângulos Laterais (Low Shelf/Bell e High Shelf/Bell) ---
    // *********** A. Low Band (Lado Esquerdo) ***********
    // Ponto de desvio angular (Âncora Y, usando a mesma lógica proporcional)
    const float lowAngleX = static_cast<float>(this->lowShelfBellButton.getBounds().getCentreX());
    // Reutilizamos a lógica de deslocamento da base do EQ visual para manter o alinhamento Y
    const float lowAngleY = static_cast<float>(this->lowShelfBellButton.getBottom()) + verticalStopOffset;

    // 1. Segmento Diagonal: Canto Inferior Direito Low -> Âncora
    g.drawLine(lowArea.getBottomRight().getX(), lowArea.getBottomRight().getY(),
        lowAngleX, lowAngleY,
        contourThickness);

    // 2. Segmento Horizontal: Âncora Low -> Borda Esquerda do Painel
    // (X1=Âncora, X2=Borda Esquerda do mainPanelArea, forçando desenho da direita para a esquerda)
    g.drawLine(lowAngleX, lowAngleY,
        mainPanelArea.getX(), lowAngleY,
        contourThickness);

    // *********** B. High Band (Lado Direito) ***********
    // Ponto de desvio angular (Âncora Y, garantindo o mesmo Y da Low Band)
    const float highAngleX = static_cast<float>(this->highShelfBellButton.getBounds().getCentreX());
    const float highAngleY = static_cast<float>(this->highShelfBellButton.getBottom() + verticalStopOffset); // Usando o mesmo offset Y!

    // 1. Segmento Diagonal: Canto Inferior Esquerdo High -> Âncora
    g.drawLine(highArea.getBottomLeft().getX(), highArea.getBottomLeft().getY(),
        highAngleX, highAngleY,
        contourThickness);

    // 2. Segmento Horizontal: Âncora High -> Borda Direita do Painel
    // (X1=Âncora, X2=Borda Direita do mainPanelArea, forçando desenho da esquerda para a direita)
    g.drawLine(highAngleX, highAngleY,
        mainPanelArea.getRight(), highAngleY,
        contourThickness);


    // LABELS DAS SESSÕES DO EQ
    g.setColour(juce::Colours::white.withAlpha(0.4f));
    g.setFont(juce::Font(11.0f).withStyle(juce::Font::bold));

    g.drawText("LOW",
        lowBandContainer.getX() - 40,
        lowBandContainer.getBottom() - 80,
        lowBandContainer.getWidth(), 15,
        juce::Justification::centred, false);

    g.drawText("LOW-MID",
        lowMidBandContainer.getX(),
        lowMidBandContainer.getBottom() - 10,
        lowMidBandContainer.getWidth(), 15,
        juce::Justification::centred, false);

    g.drawText("HIGH-MID",
        highMidBandContainer.getX(),
        highMidBandContainer.getBottom() - 10,
        highMidBandContainer.getWidth(), 15,
        juce::Justification::centred, false);

    g.drawText("HIGH",
        highBandContainer.getX() + 40,
        highBandContainer.getBottom() - 80,
        highBandContainer.getWidth(), 15,
        juce::Justification::centred, false);
    ...
    g.setColour(juce::Colours::white.withAlpha(0.6f)); // Cor dos ícones
    juce::PathStrokeType strokeType(1.0f); // Espessura da linha (ajuste conforme necessário)

// Função auxiliar para desenhar o ícone Path
    auto drawQPathIcon = [&](juce::Component& knob, bool isHighQ, juce::Justification justification)
        {
            auto bounds = knob.getBounds();
            int iconSize = 15;   // Tamanho do ícone (largura/altura)
            int paddingX = 8;   // *** NOVO: Padding horizontal maior para afastar (ex: 15px) ***
            int paddingY = 5;    // Padding vertical (distância da borda do knob)

            juce::Rectangle<int> iconArea;

            // Calcula o Y para ALINHAR AO BOTTOM do knob
            int iconBottomY = bounds.getBottom() - paddingY;
            int iconTopY = iconBottomY - iconSize; // Y de topo é o Y de base menos a altura do ícone

            if (justification == juce::Justification::centredLeft)
            {
                // Canto Inferior Esquerdo (Low Q)
                iconArea = juce::Rectangle<int>(bounds.getX() - paddingX,
                    iconTopY,
                    iconSize, iconSize);
            }
            else // centredRight (High Q)
            {
                // Canto Inferior Direito
                iconArea = juce::Rectangle<int>(bounds.getRight() + paddingX - iconSize,
                    iconTopY,
                    iconSize, iconSize);
            }

            // A área da imagem para o path é baseada em iconArea
            juce::Path qPath = isHighQ ? createHighQPath(iconArea) : createLowQPath(iconArea);
            g.strokePath(qPath, strokeType);
        };


// --- Low-Mid Q ---
// Canto Esquerdo (Q Baixo/Largo)
drawQPathIcon(lowMidQSlider, false, juce::Justification::centredLeft); 
// Canto Direito (Q Alto/Estreito)
drawQPathIcon(lowMidQSlider, true, juce::Justification::centredRight); 

// --- High-Mid Q ---
// Canto Esquerdo (Q Baixo/Largo)
drawQPathIcon(highMidQSlider, false, juce::Justification::centredLeft); 
// Canto Direito (Q Alto/Estreito)
drawQPathIcon(highMidQSlider, true, juce::Justification::centredRight);
    ...
    //Desenha o PLUGIN TITTLE
    g.setColour(juce::Colour(0xFF507B88));
    g.setFont(getFrankRuehlFont(50.0f));
    g.drawFittedText("TeLeQ", titleArea,
    juce::Justification::centred, 1);
...
void TeLeQAudioProcessorEditor::resized()
{
    ...
    backgroundImage = juce::ImageFileFormat::loadFrom(
        BinaryData::backgroundGradient_png,
        BinaryData::backgroundGradient_pngSize
    );
    ...
    // =========================================================================
    // === 6. CALCULAR TAMANHOS DOS KNOBS (ALTERADO) ===
    // =========================================================================
