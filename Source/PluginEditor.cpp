...
// INPUT GAIN
    ...
    // Input/Output Gain
    inputGainSlider.paramType = CustomSlider::Gain;
    outputGainSlider.paramType = CustomSlider::Gain;

    // EQ Sliders (Low Band)
    lowGainSlider.paramType = CustomSlider::Gain;
    lowFreqSlider.paramType = CustomSlider::Frequency;

    // EQ Sliders (Low-Mid Band)
    lowMidGainSlider.paramType = CustomSlider::Gain;
    lowMidFreqSlider.paramType = CustomSlider::Frequency;
    lowMidQSlider.paramType = CustomSlider::QFactor;

    // EQ Sliders (High-Mid Band)
    highMidGainSlider.paramType = CustomSlider::Gain;
    highMidFreqSlider.paramType = CustomSlider::Frequency;
    highMidQSlider.paramType = CustomSlider::QFactor;

    // EQ Sliders (High Band)
    highGainSlider.paramType = CustomSlider::Gain;
    highFreqSlider.paramType = CustomSlider::Frequency;

    // Filtros
    hpfSlider.paramType = CustomSlider::Frequency;
    lpfSlider.paramType = CustomSlider::Frequency;

    // Painel Drive
    driveGainSlider.paramType = CustomSlider::Percentage;
    driveMixSlider.paramType = CustomSlider::Percentage;

    // Painel Telefy
    telefyAmountSlider.paramType = CustomSlider::Percentage;
    telefyToneSlider.paramType = CustomSlider::Frequency;
    telefyIntensitySlider.paramType = CustomSlider::QFactor;
    ...
    for (auto* comp : getComps())
    {
        if (auto* slider = dynamic_cast<CustomSlider*>(comp))
        {
            ...
            label->setFont(juce::Font(15.0f).withStyle(juce::Font::bold));

            auto* rawLabel = label.get();

            // 3. Atualização de Texto (sem mexer em visibilidade)
            slider->onValueChange = [this, slider, rawLabel]() {
                auto textoFinal = slider->getTextFromValue(slider->getValue());
                if (slider->getName().contains("TelefyFreq")) {
                    auto val = slider->getValue();
                    if (val < 33.0) textoFinal = "Warm";
                    else if (val < 66.0) textoFinal = "Classic";
                    else textoFinal = "Broken";
                }
                rawLabel->setText(textoFinal, juce::dontSendNotification);
                };

            // 4. Controle de Visibilidade com NOVOS callbacks
            slider->onDragStart = [rawLabel]() {
                rawLabel->setVisible(true);
                };

            slider->onDragEnd = [rawLabel]() {
                rawLabel->setVisible(false);
                };

            // 5. Adicionar SEM força de visibilidade
            addChildComponent(rawLabel);
            rawLabel->setAlwaysOnTop(true);
            sliderLabels[slider] = std::move(label);
        }
...
void TeLeQAudioProcessorEditor::resized()
{
    ...
    for (auto const& [slider, label] : sliderLabels)
    {
        // Pega o layout do LookAndFeel para saber onde o círculo termina
        auto layout = slider->getLookAndFeel().getSliderLayout(*slider);
        auto knobArea = layout.sliderBounds;

        // NOVO: Calcular tamanho do texto dinamicamente
        auto textValue = slider->getTextFromValue(slider->getValue());
        auto font = label->getFont();
        int textWidth = font.getStringWidth(textValue) + 12; // 12px padding (6px cada lado)
        int textHeight = 20;

        int maxWidth = 150;
        if (auto* customSlider = dynamic_cast<CustomSlider*>(slider))
        {
            if (customSlider->paramType == CustomSlider::Percentage)
            {
                maxWidth = 190;  // Aumentar de 150 para 180 para percentuais
            }
        }

        // Ajuste: Se o texto for muito comprido, expande até 150px max
        textWidth = juce::jmin(textWidth, maxWidth);

        // NOVO: Centralizar o label em relação ao slider
        label->setBounds(
            slider->getX() + (slider->getWidth() - textWidth) / 2,  // Centralizado em X
            slider->getY() + knobArea.getBottom() + 2,
            textWidth,
            textHeight
        );
    }
    ...
    for (auto const& [slider, label] : sliderLabels)
    {
        // Se for um dos sliders dos painéis laterais (horizontais)
        if (slider == &driveGainSlider || slider == &driveMixSlider ||
            slider == &telefyAmountSlider || slider == &telefyToneSlider)
        {
            auto textValue = slider->getTextFromValue(slider->getValue());
            auto font = label->getFont();
            int textWidth = font.getStringWidth(textValue) + 16; // 16px padding
            int textHeight = 24;

            // Para percentuais, aumentar largura máxima
            int maxWidth = 180;
            if (auto* customSlider = dynamic_cast<CustomSlider*>(slider))
            {
                if (customSlider->paramType == CustomSlider::Percentage)
                {
                    maxWidth = 190;  // Ainda maior para percentuais
                }
            }

            textWidth = juce::jmin(textWidth, maxWidth);

            // Posicionar ACIMA do slider (porque são horizontais)
            label->setBounds(
                slider->getX() + (slider->getWidth() - textWidth) / 2,  // Centralizado em X
                slider->getY() + textHeight,  // ACIMA do slider
                textWidth,
                textHeight
            );
        }
